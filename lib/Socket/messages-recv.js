...
export const makeMessagesRecvSocket = (config) => {
    ...
    const sendMessageAck = async ({
        ...
        if (!!attrs.type &&
            (tag !== 'message' || getBinaryNodeChild({
                tag,
                attrs,
                content
        ...
        if (tag === 'message' && getBinaryNodeChild({
                tag,
                attrs,
                content
            }, 'unavailable')) {
    ...
    const handleGroupNotification = (participant, child, msg) => {
        ...
        switch (child?.tag) {
            ...
            case 'leave':
                ...
                const participants = getBinaryNodeChildren(child, 'participant').map(p => (p.attrs.phone_number || p.attrs.jid));
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                authState.creds.advSecretKey = (await hkdf(identityPayload, 32, {
                    info: 'adv_secret'
                })).toString('base64');
                await query({
                    tag: 'iq',
    ...
    const handleMessage = async (node) => {
        ...
        if (getBinaryNodeChild(node, 'unavailable') && !encNode) {
            ...
            const {
                key
            } = decodeMessageNode(node, authState.creds.me.id, authState.creds.me.lid || '').fullMessage;
